...
ALLOWED_GROUPS = [
    -1001234567890,  -1003545109479
    -1009876543210,  -1003887591413
]

def chat_allowed(message: Message):
    return (
...
async def bunker_cmd(message: Message):
    ...
    if message.chat.id in bunker_games:
        await safe_answer(
            message,
            "❌ Игра уже создана."
        )
    ...
    bunker_games[message.chat.id] = {
        "creator": message.from_user.id,
        "players": [message.from_user.id],
        "roles": {},
        "started": False,
        "scenario": random.choice(BUNKER_SCENARIOS),
        "bank": 0
    }
    ...
    await safe_answer(
        ...
        f"🌍 Сценарий:\n"
        f"『 {bunker_games[message.chat.id]['scenario']} 』\n\n"

        f"👥 Игроков:\n"
        f"『 1/{BUNKER_MAX_PLAYERS} 』\n\n"

        "📝 Войти:\n"
        "<code>/joinbunker</code>"
    )
    
@dp.message(Command("joinbunker"))
async def join_bunker(message: Message):

    game = bunker_games.get(message.chat.id)
    ...
    if len(game["players"]) >= BUNKER_MAX_PLAYERS:
        await safe_answer(
            message,
            "❌ Бункер заполнен."
        )
    ...
    await safe_answer(
        message,
        f"☢️ {message.from_user.full_name} вошёл в бункер.\n"
        f"👥 Игроков: {len(game['players'])}/{BUNKER_MAX_PLAYERS}"
    )
    
@dp.message(Command("startbunker"))
async def start_bunker(message: Message):
    ...
    if len(game["players"]) < BUNKER_MIN_PLAYERS:
        await safe_answer(
            message,
    ...
    for player in game["players"]:
        ...
        role = {
            "job": random.choice(BUNKER_JOBS),
            "health": random.choice(BUNKER_HEALTH),
            "skill": random.choice(BUNKER_SKILLS),
            "phobia": random.choice(BUNKER_PHOBIAS),
    ...
    await safe_answer(
        ...
        "🗣 Фаза обсуждения началась.\n"
        "⏳ Время: 60 секунд."
    )
    
@dp.message(Command("bunkerhelp"))
async def bunker_help(message: Message):
    await safe_answer(
        message,
        """
╔════════════════════╗
...
🗳 /vote @игрок
Проголосовать против игрока
"""
    )
    
@dp.message(Command("leavebunker"))
async def leave_bunker(message: Message):
    ...
    game["players"].remove(message.from_user.id)

    if message.from_user.id in game["roles"]:
        del game["roles"][message.from_user.id]
...
@dp.message(Command("bunkerplayers"))
async def bunker_players(message: Message):
    ...
    for i, player in enumerate(game["players"], 1):
        ...
        try:
            user = await bot.get_chat(player)
...
@dp.message(Command("bunkerend"))
async def bunker_end(message: Message):
    ...
    del bunker_games[message.chat.id]
...
@dp.message(Command("bunkerstats"))
async def bunker_stats_cmd(message: Message):

    user = str(message.from_user.id)

    stats = bunker_stats.get(
        user,
        {
            "games": 0,
            "wins": 0,
...
async def vote_cmd(message: Message):
    ...
    bunker_votes.setdefault(message.chat.id, {})
    bunker_votes[message.chat.id][message.from_user.id] = target

    votes = list(
        bunker_votes[message.chat.id].values()
    ).count(target)
...
BUNKER_MIN_PLAYERS = 4
BUNKER_MAX_PLAYERS = 10
...
BUNKER_EVENTS = [
    ...
    "💧 Закончилась вода",
    "☢️ Радиация увеличилась",
    "🦠 Обнаружен вирус",
]

async def bunker_round(chat_id):
    ...
    event = random.choice(BUNKER_EVENTS)
